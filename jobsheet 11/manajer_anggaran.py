...
class AnggaranHarian:
    ...
    def hapus_transaksi(self, id_transaksi: int) -> bool:
        ...
        print(f"[DEBUG] Memanggil hapus_transaksi dengan id_transaksi: {id_transaksi}")  # Debug statement
        sql = "DELETE FROM transaksi WHERE id = ?"
        params = (id_transaksi,)
        
        try:
            print(f"[DEBUG] Menjalankan SQL: {sql} dengan params: {params}")  # Debug statement
            result = database.execute_query(sql, params)
            if result is not None:
                print("[DEBUG] Penghapusan berhasil.")  # Debug statement
                return True
            else:
                print("[DEBUG] Penghapusan gagal, result adalah None.")  # Debug statement
                return False
        except Exception as e:
            print(f"[ERROR] Terjadi kesalahan saat menghapus transaksi: {e}")  # Error statement
            return False


    def get_dataframe_transaksi(self, filter_tanggal: datetime.date | None = None) -> pd.DataFrame:
        query = "SELECT id, tanggal, kategori, deskripsi, jumlah FROM transaksi" # Tambahkan 'id'
        params = None
        ...
        if not df.empty:
            ...
            # Pastikan kolom 'id' ada untuk digunakan di frontend
